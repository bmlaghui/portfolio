          ...
          <main class="modal-main">
            ...
            <form (ngSubmit)="save()" class="liquid-form" #blogForm="ngForm">
              <div class="tab-pane reveal" *ngIf="activeTab() === 'fr'">
                ...
                <div class="form-group full"><label>Contenu (FR)</label><textarea [(ngModel)]="formPost.content" name="content" required class="cyber-input area" rows="15"></textarea></div>
              ...
              <div class="tab-pane reveal" *ngIf="activeTab() === 'en'">
                <div class="form-group large"><label>Title (EN)</label><input type="text" [(ngModel)]="formPost.titleEn" name="te" required class="cyber-input"></div>
                <div class="form-group large"><label>Summary (EN)</label><input type="text" [(ngModel)]="formPost.summaryEn" name="se" required class="cyber-input"></div>
                <div class="form-group full"><label>Content (EN)</label><textarea [(ngModel)]="formPost.contentEn" name="ce" required class="cyber-input area" rows="15"></textarea></div>
              ...
              <div class="tab-pane reveal" *ngIf="activeTab() === 'config'">
                ...
                <div class="form-grid" style="display: grid; grid-template-columns: 1fr 1fr; gap: 2rem;">
                  <div class="form-group"><label>Temps lecture</label><input type="number" [(ngModel)]="formPost.readTime" name="read" class="cyber-input"></div>
                ...
                <footer class="modal-footer-final"><button type="button" class="btn-cancel" (click)="closeModal()">ABANDONNER</button><div class="spacer"></div><button type="submit" class="btn-submit-cyber" [disabled]="blogForm.invalid || !formPost.imageUrl">PUBLIER ARCHIVE</button></footer>
              </div>
            </form>
          </main>
        </div>
...
export class AdminBlogComponent implements OnInit {
  ...
  openModal(p?: BlogPost) { this.activeTab.set('fr'); if (p) { this.formPost = { ...p, tags: [...(p.tags || [])] }; this.editingId.set(p.id!); } else { this.formPost = this.resetForm(); this.editingId.set(null); } this.showModal.set(true); }
  closeModal() { this.showModal.set(false); }
  resetForm(): BlogPost { return { title: '', summary: '', content: '', titleEn: '', summaryEn: '', contentEn: '', slug: '', published: false, tags: [], readTime: 5 }; }
  tagsString() { return this.formPost.tags?.join(', ') || ''; }
  setTags(val: string) { this.formPost.tags = val.split(',').map(s => s.trim()).filter(s => !!s); }
  save() { 
    if (!this.formPost.slug) this.formPost.slug = this.formPost.title.toLowerCase().replace(/ /g, '-').replace(/[^\w-]+/g, ''); 
